...
[배포 링크](https://infinite-scroll-two-lemon.vercel.app/)

![2024-10-306 04 56-ezgif com-video-to-gif-converter](https://github.com/user-attachments/assets/b25211b5-bd5f-485c-8191-04129a49dc61)

### Feature

1.  **가격 누적 합계 표시**
    - 상품 리스트 하단에 로드된 데이터(상품)의 가격 누적 합계가 나타난다.
    - 첫 페이지 로드 전에는 "Total Price: $0"이며, 데이터 로드 완료 후 누적 합계가 업데이트된다.

2. **무한 스크롤 조건**
    - 로딩 UI가 화면에 보일 때만 다음 페이지가 자동으로 로드된다.
    - 로딩 UI가 화면에 보이지 않을 경우 (ex. 상품 카드 또는 Total Price 문자열 만 보이는 상태) 새로운 페이지를 불러오지 않는다.

3. **마지막 페이지 처리**: 마지막 페이지일 경우 로딩 UI가 표시되지 않으며, 더 이상 페이지를 불러오지 않는다.
...
[Intersection Observer API를 이용해 무한 스크롤을 구현하는 과정을 정리한 포스팅](https://hyrmzz1.tistory.com/167) 입니다 ✍🏻

---

## 과제 요구사항

- [x] 하나의 SinglePage에 Intersection Observer를 이용해 무한스크롤을 구현하세요.
- [x] 현재 가져온 상품 리스트들의 액수들의 합계를 화면에 보여주세요.

### 무한 스크롤의 조건

- [x] 페이지를 현재 보여주는 페이지의 최하단으로 이동 시 다음 페이지 정보를 가져오게 합니다.
- [x] 더이상 가져올 수 없는 상황이라면 더 이상 데이터를 가져오는 함수를 호출하지 않습니다.
- [x] 로딩 시 로딩 UI가 보여아 합니다.

### 과제 유의 사항

- [x] React + 함수형 컴포넌트를 사용해서 개발해주세요.
- [x] 제공해드린 Mock 데이터는 수정 및 추가가 가능합니다.
- [x] 무한스크롤 관련된 라이브러리 사용 절대 금지
- [x] 비동기 상태 관리 라이브러리 사용 절대 금지 (ex. tanstack-query)
